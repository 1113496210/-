# -*- coding: utf-8 -*-
"""
融合监控系统（启动雷达 + 四通道 + 动态日报版）
通知优化：
- Early静默入库，不发送TG/邮箱
- 同币阶段锁，每个阶段仅推送一次
- 爆发限制：确认类每轮最多5个，强启动最多3个
- 速报24小时去重
- 启动确认/强启动同币一次
- 失败完全静默，仅记录数据库
- 日报增加通道统计和生命周期字段
...
with db_conn:
    ...
    """)
    db_conn.execute("""
        CREATE TABLE IF NOT EXISTS stage_lock (
            symbol TEXT,
            stage TEXT,
            sent_at TIMESTAMP,
    ...
    """)
    # 迁移：为 early_signal_log 补充生命周期字段
    cur = db_conn.execute("PRAGMA table_info(early_signal_log)")
    existing_cols = {row[1] for row in cur.fetchall()}
    new_cols = {
        "surge_time": "TIMESTAMP",
        "strong_time": "TIMESTAMP",
        "trend_time": "TIMESTAMP",
        "trend_alert_time": "TIMESTAMP",
        "final_status": "TEXT",
        "entry_price": "REAL",
    }
    for col, col_type in new_cols.items():
        if col not in existing_cols:
            db_conn.execute(f"ALTER TABLE early_signal_log ADD COLUMN {col} {col_type}")
...
def should_alert_window(symbol, direction, channel, hours):
    cutoff = now_beijing() - timedelta(hours=hours)
    with db_lock:
        cur = db_conn.execute(
            "SELECT 1 FROM sent_log WHERE symbol=? AND direction=? AND channel=? AND sent_at > ?",
...
def mark_alerted_window(symbol, direction, channel):
    mark_alerted(symbol, direction, channel)

def should_stage_alert(symbol, stage):
    with db_lock:
        cur = db_conn.execute(
            "SELECT 1 FROM stage_lock WHERE symbol=? AND stage=?",
            (symbol, stage))
        return cur.fetchone() is None

def mark_stage_alerted(symbol, stage):
    with db_lock:
        db_conn.execute(
            "INSERT OR REPLACE INTO stage_lock (symbol, stage, sent_at) VALUES (?,?,?)",
            (symbol, stage, now_beijing()))
        db_conn.commit()

def update_early_lifecycle(early_id, **kwargs):
    allowed = {
        "surge_time", "confirm_time", "strong_time", "trend_time",
        "trend_alert_time", "final_status", "entry_price"
    }
...
def should_trend_alert(symbol):
    ...
    first_at = parse_ts(first_at_str)
    last_sent = parse_ts(sent_at_str)
    if (now_beijing() - first_at).total_seconds() > 48 * 3600:
        return False
    if (now_beijing() - last_sent).total_seconds() < TREND_DEDUP_HOURS * 3600:
        return False
...
def mark_trend_alerted(symbol):
    with db_lock:
        ...
        if row is None:
            ...
            first_at_dt = parse_ts(first_at)
            if (now_beijing() - first_at_dt).total_seconds() > 48 * 3600:
                db_conn.execute(
                    "INSERT INTO trend_log VALUES (?,?,?,?)",
                    (symbol, 1, now_beijing().isoformat(), now_beijing().isoformat()))
            else:
...
def send_email_text(subject, content):
    ...
    try:
        ...
        server.sendmail(QQ_EMAIL, [e.strip() for e in EMAIL_TO.split(",") if e.strip()], msg.as_string())
        return True
    except Exception as e:
...
def notify_signal(sig):
    # 阶段锁检查：确认阶段同币一次
    if not should_stage_alert(sig["symbol"], "confirm"):
        return False
    # 若为强启动，检查强启动锁
    if sig.get("score", 0) >= STRONG_SCORE_MIN:
        if not should_stage_alert(sig["symbol"], "strong"):
            return False
        mark_stage_alerted(sig["symbol"], "strong")
    mark_stage_alerted(sig["symbol"], "confirm")
    ...
    notify_tg_email(text, f"🚀 [正式启动] {sig['symbol']} 评分{score}/{max_score}", channel="confirm", symbol=sig["symbol"])
    return True

def notify_surge(symbol, price, start_pct):
    # 速报24小时去重
    if not should_alert_window(symbol, "LONG", "surge", hours=24):
        return False
    mark_alerted_window(symbol, "LONG", "surge")
    ...
    notify_tg_email(text, f"⚡ [快速拉升] {symbol} +{start_pct:.2f}%", channel="surge", symbol=symbol)
    return True
...
def notify_trend(symbol, price, change_24h, bull_count, at_high_pct, trend_count):
    ...
    notify_tg_email(text, f"📈 [趋势信号] {symbol}", channel="trend", symbol=symbol)
...
def notify_failure(symbol, price, reason, drawdown_pct):
    # 失败完全静默，仅记录数据库（在调用处已经记录）
    logger.info(f"❌ 启动失败记录: {symbol} {reason}")
...
pending_confirm_notifications = []  # 新增：待发送确认信号列表
...
def get_4h_and_day_position(symbol):
    try:
        ...
        if not k4 or not kd: return 0,0,0
        price = float(kd[-1][4])
        high4 = max(float(x[2]) for x in k4)
        high24 = max(float(x[2]) for x in kd)
        low24 = min(float(x[3]) for x in kd)
        day_range_pos = (price - low24) / (high24 - low24) if high24 > low24 else 0.5
        dist4 = (high4 - price) / price if price > 0 else 1.0
        change1h = (float(kd[-1][4]) - float(kd[-2][4])) / float(kd[-2][4]) if len(kd)>=2 and float(kd[-2][4])>0 else 0
...
def compute_chase_risk(symbol, price):
    ...
    try:
        ...
        if change1h > MAX_1H_CHANGE: risk += 35
        elif change1h > MAX_1H_CHANGE*0.75: risk += 20
        if 0 < dist4 < CHASE_4H_HIGH_DISTANCE: risk += 25
        if day_pos >= CHASE_DAY_RANGE_TOP: risk += 20
        vwap = get_short_term_vwap(symbol, 60)
        if vwap and price > vwap:
            premium = (price - vwap) / vwap
            if premium > 0.02: risk += 20
            elif premium > 0.01: risk += 10
        return min(100, risk)
    except: return risk
...
def get_market_state(symbols=None):
    try:
        ...
        if len(btc) >= 16:
            btc_change_15m = (float(btc[-1][4]) - float(btc[-16][4])) / float(btc[-16][4])
        else: btc_change_15m = 0.0
    except: btc_change_15m = 0.0
    down_ratio = 0.0
    if symbols:
        ...
        if sample:
            def _one(s):
                try:
                    ...
                    if len(k) < 16: return 0
                    return 1 if float(k[-1][4]) < float(k[-16][4]) else 0
                except: return 0
            with ThreadPoolExecutor(max_workers=15) as ex:
                vals = list(ex.map(_one, sample))
            down_ratio = sum(vals) / len(vals)
    return {"btc_change_15m": btc_change_15m, "market_down_ratio": down_ratio}

def insert_early_signal(symbol, trigger_price, trigger_high, trigger_low, start_pct, early_score, volume_ratio):
    with db_lock:
        cur = db_conn.execute("""
...
def is_symbol_in_cooldown(symbol):
    ...
    with db_lock:
        cur = db_conn.execute("""
            SELECT cooldown_until FROM early_signal_log
            WHERE symbol=? AND cooldown_until IS NOT NULL
            ORDER BY id DESC LIMIT 1
        """, (symbol,))
        row = cur.fetchone()
    if not row or not row[0]: return False
    try: return parse_ts(row[0]) > now
    except: return False

def add_tracking_event(symbol, early_id, event_type, price=None, change_pct=None, drawdown_pct=None, progress_class=None, note=None):
    with db_lock:
        db_conn.execute("""
            INSERT INTO tracking_events
            (symbol, early_id, event_time, event_type, price, change_pct, drawdown_pct, progress_class, note)
            VALUES (?,?,?,?,?,?,?,?,?)
        """, (
...
def update_early_row(early_id, **kwargs):
    allowed = {
        "stage","result","confirm_time","confirm_price","price_3m","price_6m",
        "price_15m","max_price","min_price","max_gain","max_drawdown",
        "progress_class","chase_risk","formal_score","fail_reason","cooldown_until"
    }
    kwargs = {k:v for k,v in kwargs.items() if k in allowed}
    ...
    values.append(early_id)
    with db_lock:
        db_conn.execute(f"UPDATE early_signal_log SET {assignments} WHERE id=?", values)
        db_conn.commit()

# ===================== 启动雷达 =====================
def scan_trigger(sym):
    try:
        if sym not in high_volume_symbols: return
        if sym in pending_early or sym in active_tracking: return
        existing = get_existing_early(sym, EARLY_DEDUP_MINUTES)
        if existing or is_symbol_in_cooldown(sym): return
        ...
        if sb["daily_round"] >= MAX_DAILY_ROUND: return
        ...
        start_pct = (highs[-1] - lows[-3]) / lows[-3] * 100 if lows[-3] > 0 else 0
        if not hh or not (EARLY_HH_MIN <= start_pct <= EARLY_HH_MAX): return

        current1 = get_klines(sym, "1m", 12)
        if len(current1) < 6: return
        last = current1[-1]; prev = current1[-2]
        last_o, last_h, last_l, last_c, last_v = map(float, [last[1],last[2],last[3],last[4],last[5]])
        prev_h = float(prev[2])
        ...
        close_pos = (last_c - last_l)/high_low_range if high_low_range>0 else 0.5
        ...
        score = 0
        score += 1
        if body_pct >= EARLY_1M_BODY_MIN: score += 1
        if vol_ratio_1m >= EARLY_1M_VOL_MULT: score += 1
        if close_pos >= EARLY_CLOSE_POSITION or break_ok: score += 1

        if score < 3: return

        price_now = last_c
        early_id = insert_early_signal(sym, price_now, highs[-1], lows[-3], start_pct, score, vol_ratio_1m)
        entry = {
            "id": early_id,
            "symbol": sym,
        ...
        sb["source"] = "early"
        logger.info(f"🟡 Early入库观察: {sym}")
        logger.info(f"🟡 启动预警: {sym} HH={start_pct:.2f}% score={score}/4 vol={vol_ratio_1m:.2f}x")

        # 速报处理：24小时去重
        if start_pct >= SURGE_HH_PCT and time.time() - startup_time >= SILENT_PERIOD_SEC:
            if should_alert_window(sym, "LONG", "surge", hours=24):
                if notify_surge(sym, price_now, start_pct):
                    mark_alerted_window(sym, "LONG", "surge")
                    update_early_lifecycle(early_id, surge_time=now_beijing())
    except Exception as e:
...
def analyze_symbol(symbol, early=None, market_state=None):
    try:
        ...
        if early:
            current = closes5[-1]
            post_start_pct = (current - early["trigger_price"]) / early["trigger_price"] if early["trigger_price"] else 0
        ...
        recent15_ok = (bullish >= 2 and float(last3[-1][4]) > float(last3[-2][2]))
        oi_ok = oi_change > 0
        ...
        if one_hour_change > MAX_1H_CHANGE:
            return {"symbol": symbol, "direction": "LONG", "rejected": True, "reject_reason": "1h涨幅超过追高阈值", "price": price, "score": score, "max_score": 6, "chase_risk": chase_risk}
        if chase_risk >= CHASE_MAX_RISK:
            return {"symbol": symbol, "direction": "LONG", "rejected": True, "reject_reason": f"追高风险{chase_risk}/100", "price": price, "score": score, "max_score": 6, "chase_risk": chase_risk}
        if market_bad:
            return {"symbol": symbol, "direction": "LONG", "rejected": True, "reject_reason": "全市场下跌占比超过60%", "price": price, "score": score, "max_score": 6, "chase_risk": chase_risk}
        quality = "强启动" if score >= STRONG_SCORE_MIN else "正式启动" if score >= FORMAL_SCORE_MIN else "弱启动"
        ...
        return {
            "symbol": symbol, "direction": "LONG", "score": score, "max_score": 6,
            "quality_label": quality, "price": price, "oi_change": oi_change,
            ...
            "chase_risk": chase_risk, "btc_change_15m": market_state["btc_change_15m"],
            "market_down_ratio": market_state["market_down_ratio"], "rejected": False
        }
    except Exception as e:
...
def process_early_validation():
    ...
    for sym in list(pending_early.keys()):
        ...
        try:
            ...
            gain = (price - entry["trigger_price"]) / entry["trigger_price"] if entry["trigger_price"] > 0 else 0
            drawdown = (entry["max_price"] - price) / entry["max_price"] if entry["max_price"] > 0 else 0
            ...
            update_early_row(entry["id"], max_price=entry["max_price"], min_price=entry["min_price"], max_gain=entry["max_gain_seen"], max_drawdown=entry["max_drawdown_seen"], progress_class=progress_class)

            # 硬失败
            if price < entry["trigger_low"]:
                reason = "跌破启动基准低点"
                cooldown = now_beijing() + timedelta(seconds=EARLY_FAIL_COOLDOWN_SEC)
                update_early_row(entry["id"], result="FAILED", stage="FAILED", fail_reason=reason, cooldown_until=cooldown)
                add_tracking_event(sym, entry["id"], "FAILED", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note=reason)
                logger.info(f"❌ Early失败: {sym} {reason}")
                with early_lock:
            ...
            if drawdown >= EXTREME_DRAWDOWN:
                reason = "启动后最大回撤达到5%"
                cooldown = now_beijing() + timedelta(seconds=LONG_FAIL_COOLDOWN_SEC)
                update_early_row(entry["id"], result="FAILED", stage="FAILED", fail_reason=reason, cooldown_until=cooldown)
                add_tracking_event(sym, entry["id"], "EXTREME_FAIL", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note=reason)
                logger.info(f"❌ Early极端回撤失败: {sym} {reason}")
                with early_lock:
                    pending_early.pop(sym, None)
                continue
            ...
            if elapsed >= FAST_CHECK_3M_SEC and not entry["check_3m_done"]:
                ...
                update_early_row(entry["id"], stage="VALIDATING_6M", price_3m=price)
                add_tracking_event(sym, entry["id"], "3M_CHECK", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note="3分钟承接检查")

            if elapsed >= FAST_CHECK_6M_SEC and not entry["check_6m_done"]:
                entry["check_6m_done"] = True
                fast_ok = (gain >= FAST_MIN_PROGRESS and new_high and price >= entry["trigger_price"])
                slow_ok = (gain >= SLOW_MIN_PROGRESS and price >= entry["trigger_price"] and drawdown < SOFT_DRAWDOWN*1.5)
                if fast_ok or (slow_ok and progress_class in ("正常推进","强推进","爆发")):
                    entry["stage"] = "CONFIRMED"
                    entry["active"] = True
                    entry["confirm_price"] = price
                    update_early_row(entry["id"], result="CONFIRMED", stage="CONFIRMED", confirm_time=now_beijing(), confirm_price=price, price_6m=price, progress_class=progress_class)
                    add_tracking_event(sym, entry["id"], "CONFIRMED", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note="6分钟启动确认")
                    ...
                    if sig and sig.get("rejected"):
                        ...
                        update_early_row(entry["id"], formal_score=sig.get("score",0), chase_risk=sig.get("chase_risk",0), fail_reason=sig.get("reject_reason","风险过滤"))
                    elif sig and sig.get("score",0) >= FORMAL_SCORE_MIN:
                        # 收集信号，统一由主循环发送
                        pending_confirm_notifications.append(sig)
                    else:
                        basic_sig = {
                            "symbol": sym, "direction":"LONG", "price":price,
                            "score": sig.get("score",0) if sig else 0, "max_score":6,
                        ...
                        pending_confirm_notifications.append(basic_sig)
                    ...
                    update_early_row(entry["id"], stage="WATCH_15M", price_6m=price, progress_class=progress_class)
                    add_tracking_event(sym, entry["id"], "6M_WEAK", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note="6分钟未达到快速确认，延长到15分钟")
            ...
            if elapsed >= SLOW_CHECK_15M_SEC and not entry["check_15m_done"]:
                ...
                slow_confirm = (gain >= SLOW_MIN_PROGRESS and slow_new_high and drawdown < SOFT_DRAWDOWN*1.5)
                if slow_confirm:
                    entry["stage"] = "CONFIRMED"
                    entry["active"] = True
                    entry["confirm_price"] = price
                    update_early_row(entry["id"], result="CONFIRMED", stage="CONFIRMED", confirm_time=now_beijing(), confirm_price=price, progress_class=progress_class)
                    sig = analyze_symbol(sym, entry, market_state_cache)
                    if sig and sig.get("rejected"):
                        notify_tg_only(
                        ...
                        if sig:
                            ...
                            pending_confirm_notifications.append({
                                "symbol": sym, "direction":"LONG", "price": price,
                                "score": 0, "max_score":6,
                                "quality_label":"慢启动确认",
                                "oi_change": 0, "volume_ratio": 0, "funding": 0,
                                "post_start_pct": gain*100,
                                "progress_class": progress_class,
                                "chase_risk": 0,
                            })
                    ...
                    cooldown = now_beijing() + timedelta(seconds=EARLY_FAIL_COOLDOWN_SEC)
                    update_early_row(entry["id"], result="FAILED", stage="FAILED", fail_reason=reason, cooldown_until=cooldown)
                    add_tracking_event(sym, entry["id"], "TIMEOUT_FAIL", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note=reason)
                    logger.info(f"❌ Early超时失败: {sym} {reason}")
                    with early_lock:
                        pending_early.pop(sym, None)
...
def track_active():
    for sym in list(active_tracking.keys()):
        ...
        try:
            ...
            if isinstance(trigger_dt, str): trigger_dt = parse_ts(trigger_dt)
            elapsed_min = (now_beijing() - trigger_dt).total_seconds()/60
            k1 = get_klines(sym, "1m", PUSH_ATR_PERIOD+5)
            if not k1: continue
            price = float(k1[-1][4])
            high_now = max(float(x[2]) for x in k1[-3:])
            low_now = min(float(x[3]) for x in k1[-3:])
            prev_high = sb.get("last_recorded_high", sb.get("highest_price", sb.get("first_price") or price))
            if high_now > prev_high:
                sb["last_recorded_high"] = high_now
                sb["last_new_high_time"] = time.time()
            sb["highest_price"] = max(sb.get("highest_price", sb.get("first_price") or price), price, high_now)
            ...
            drawdown = (sb["highest_price"] - price)/sb["highest_price"] if sb["highest_price"]>0 else 0
            sb["max_gain_seen"] = max(sb.get("max_gain_seen", gain), gain)
            sb["max_drawdown_seen"] = max(sb.get("max_drawdown_seen", drawdown), drawdown)
            progress_class, _ = classify_progress(gain, atr_percent_from_1m(k1))
            if elapsed_min >= TRACKING_TIMEOUT_HOURS*60:
                update_early_row(sb["id"], result="EXPIRED", stage="EXPIRED", max_price=sb["highest_price"], min_price=sb["daily_low"], max_gain=sb["max_gain_seen"], max_drawdown=sb["max_drawdown_seen"])
                add_tracking_event(sym, sb["id"], "EXPIRED", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note="24小时超时")
                notify_tg_only(f"⏰ {sym} — 24小时跟踪结束\n当前价: {price:.8f}\n启动后: {gain*100:+.2f}%\n已撤出关注池。", channel="expired", symbol=sym)
                del active_tracking[sym]
                continue
            if price < trigger_low:
                update_early_row(sb["id"], result="FAILED", stage="FAILED", max_price=sb["highest_price"], min_price=price, max_gain=sb["max_gain_seen"], max_drawdown=sb["max_drawdown_seen"], fail_reason="正式确认后跌破启动基准低点", cooldown_until=now_beijing()+timedelta(seconds=LONG_FAIL_COOLDOWN_SEC))
                add_tracking_event(sym, sb["id"], "STRUCTURE_FAIL", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note="跌破启动基准低点")
                logger.info(f"❌ 正式跟踪失败: {sym} 跌破启动基准低点")
                del active_tracking[sym]
                continue
            if drawdown >= EXTREME_DRAWDOWN:
                update_early_row(sb["id"], result="FAILED", stage="FAILED", max_price=sb["highest_price"], min_price=price, max_gain=sb["max_gain_seen"], max_drawdown=sb["max_drawdown_seen"], fail_reason="正式确认后最大回撤达到5%", cooldown_until=now_beijing()+timedelta(seconds=LONG_FAIL_COOLDOWN_SEC))
                add_tracking_event(sym, sb["id"], "EXTREME_FAIL", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note="最大回撤达到5%")
                logger.info(f"❌ 正式跟踪极端回撤失败: {sym}")
                del active_tracking[sym]
                continue
            if drawdown >= SOFT_DRAWDOWN:
                add_tracking_event(sym, sb["id"], "SOFT_DRAWDOWN", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note="软回撤0.8%")
            prev_high_val = sb.get("last_recorded_high", trigger_price)
            if high_now > prev_high_val:
                sb["last_new_high_time"] = time.time()
                push_pct = (high_now - prev_high_val)/prev_high_val if prev_high_val>0 else 0
                if push_pct >= max(NORMAL_PUSH_MIN, atr_percent_from_1m(k1)*NORMAL_PUSH_ATR):
                    sb["push_times"] = sb.get("push_times",0)+1
                    sb["push_count"] = min(MAX_PUSH, sb.get("push_count",0)+1)
                    sb["last_recorded_high"] = high_now
                    update_early_row(sb["id"], max_price=sb["highest_price"], max_gain=sb["max_gain_seen"], max_drawdown=sb["max_drawdown_seen"], progress_class=progress_class)
                    add_tracking_event(sym, sb["id"], "NEW_HIGH", price=price, change_pct=push_pct*100, drawdown_pct=drawdown*100, progress_class=progress_class, note="有效新高推进")
                    if time.time() - startup_time >= SILENT_PERIOD_SEC:
                        notify_tg_only(f"🚀 {sym} — {progress_class}\n当前价: {price:.8f}\n启动后: {gain*100:+.2f}%\n本次新高推进: {push_pct*100:+.2f}%\n累计推进次数: {sb.get('push_times',0)}", channel="push", symbol=sym)
            if "last_new_high_time" not in sb:
                sb["last_new_high_time"] = time.time() - (elapsed_min*60)
            minutes_since_high = (time.time() - sb["last_new_high_time"])/60
            if elapsed_min >= MOMENTUM_WEAK_MINUTES and not sb.get("weak_notified") and minutes_since_high >= MOMENTUM_WEAK_MINUTES:
                notify_momentum_weak(sym, price, gain*100, MOMENTUM_WEAK_MINUTES, progress_class)
                sb["weak_notified"] = True
            if elapsed_min >= MOMENTUM_FAIL_MINUTES and minutes_since_high >= MOMENTUM_FAIL_MINUTES and gain < FAST_MIN_PROGRESS:
                reason = "30分钟无有效延续"
                update_early_row(sb["id"], result="FLAT", stage="FLAT", max_price=sb["highest_price"], min_price=sb["daily_low"], max_gain=sb["max_gain_seen"], max_drawdown=sb["max_drawdown_seen"], fail_reason=reason)
                add_tracking_event(sym, sb["id"], "FLAT_EXIT", price=price, change_pct=gain*100, drawdown_pct=drawdown*100, progress_class=progress_class, note=reason)
                logger.info(f"📊 {sym} 横盘退出")
                del active_tracking[sym]
        except Exception as e:
...
def scan_trends(symbols):
    ...
    if not target_syms: return
    def check_trend(sym):
        try:
            k1h = get_klines(sym, "1h", 24)
            if len(k1h) < 24: return None
            ...
            if (change_24h > TREND_CHANGE_MIN and bull_count >= TREND_BULL_MIN and at_high_pct > TREND_AT_HIGH_MIN):
    ...
    for r in results:
        if r and should_trend_alert(r[0]):
            ...
            with db_lock:
                cur = db_conn.execute("SELECT id FROM early_signal_log WHERE symbol=? ORDER BY id DESC LIMIT 1", (sym,))
                row = cur.fetchone()
                if row:
                    update_early_lifecycle(row[0], trend_alert_time=now_beijing())
            hits += 1
...
def scan_building_signals():
    ...
    for sym in candidates:
        try:
            ...
            notify_all(msg, channel="building", symbol=sym)
...
def generate_daily_report(report_date=None):
    ...
    with db_lock:
        early_rows = db_conn.execute("""
            SELECT
                ...
                surge_time, strong_time, trend_time, trend_alert_time,
                final_status, entry_price
            FROM early_signal_log
        ...
        channel_counts = {}
        for ch in ["surge", "confirm", "trend", "building"]:
            cnt = db_conn.execute("""
                SELECT COUNT(*) FROM sent_log
                WHERE channel=? AND sent_at>=? AND sent_at<?
            """, (ch, date_start.isoformat(), date_end.isoformat())).fetchone()[0]
    ...
    columns = [
        ...
        "最大涨幅%", "最大回撤%", "推进等级", "追高风险", "正式评分",
        "失败原因", "速报时间", "强启动时间", "趋势时间", "趋势提醒时间",
        "最终状态", "入场价"
    ]
    df = pd.DataFrame(early_rows, columns=columns)

    if not df.empty:
        numeric_cols = [
            "启动价", "3M启动幅度%", "预警评分", "初始量能比", "确认价",
            "3M价格", "6M价格", "15M价格", "最高价", "最低价",
            "最大涨幅%", "最大回撤%", "追高风险", "正式评分", "入场价"
        ]
        for col in numeric_cols:
            df[col] = pd.to_numeric(df[col], errors="coerce")

        time_cols = ["启动时间", "确认时间", "速报时间", "强启动时间", "趋势时间", "趋势提醒时间"]
        for col in time_cols:
            df[col] = pd.to_datetime(df[col], errors="coerce")

    total_early = len(df)
    confirmed = int((df["最终结果"] == "CONFIRMED").sum()) if total_early else 0
    failed = int((df["最终结果"] == "FAILED").sum()) if total_early else 0
    flat = int((df["最终结果"] == "FLAT").sum()) if total_early else 0
    pending = int((df["最终结果"] == "PENDING").sum()) if total_early else 0
    expired = int((df["最终结果"] == "EXPIRED").sum()) if total_early else 0

    if total_early:
        p3 = int(df["3M价格"].notna().sum())
        p6 = int(df["6M价格"].notna().sum())
        p15 = int(df["15M价格"].notna().sum())
        confirm_rate = confirmed / total_early * 100
    else:
        p3 = p6 = p15 = 0
        confirm_rate = 0

    funnel_data = [
        ["启动预警", total_early, 100.0],
        ["记录到3分钟", p3, p3/total_early*100 if total_early else 0],
        ["进入6分钟节点", p6, p6/total_early*100 if total_early else 0],
        ["进入15分钟节点", p15, p15/total_early*100 if total_early else 0],
        ["正式确认", confirmed, confirm_rate],
        ["失败", failed, failed/total_early*100 if total_early else 0],
        ["横盘退出", flat, flat/total_early*100 if total_early else 0],
        ["超时", expired, expired/total_early*100 if total_early else 0],
        ["仍待处理", pending, pending/total_early*100 if total_early else 0],
    ]
    funnel_df = pd.DataFrame(funnel_data, columns=["阶段", "数量", "占启动预警比例%"])
    funnel_df["占启动预警比例%"] = funnel_df["占启动预警比例%"].round(2)
    ...
    if total_early:
        ...
        if not fails.empty:
            ...
            failure_df["占失败比例%"] = (failure_df["数量"] / max(1, len(fails)) * 100).round(2)

    best_df = pd.DataFrame()
    if total_early:
        best_df = df.sort_values(["最大涨幅%", "正式评分"], ascending=[False, False]).head(10).copy()
        best_df = best_df[
            ["币种", "启动时间", "启动价", "最高价", "最大涨幅%",
             "最大回撤%", "最终结果", "正式评分", "推进等级"]
        ]

    event_columns = ["币种", "启动ID", "事件时间", "事件类型", "价格", "变化%", "回撤%", "推进等级", "说明"]
    events_df = pd.DataFrame(event_rows, columns=event_columns)
    if not events_df.empty:
        events_df["事件时间"] = pd.to_datetime(events_df["事件时间"], errors="coerce")
    ...
    with pd.ExcelWriter(fname, engine="openpyxl") as writer:
        ...
        funnel_df.to_excel(writer, sheet_name="启动漏斗", index=False)
    ...
    if not best_df.empty:
        top = best_df.iloc[0]
        top_text = f"{top['币种']}，启动后最大涨幅 {float(top['最大涨幅%']):+.2f}%"
    ...
    body = (
        ...
        f"▸ 仍待处理：{pending}\n\n"
        f"📮 通道推送统计：\n"
        f"   ⚡ 速报：{channel_counts.get('surge',0)}\n"
        f"   🚀 启动确认：{channel_counts.get('confirm',0)}\n"
        f"   📈 趋势：{channel_counts.get('trend',0)}\n"
        f"   📐 建仓：{channel_counts.get('building',0)}\n\n"
        f"🎯 预警→确认转化率：{confirm_rate:.2f}%\n"
        f"🏆 今日最佳：{top_text}\n\n"
        f"附件包含：信号明细、启动漏斗、失败分析、最佳信号、跟踪事件。"
    )

    email_ok = send_email_attach(f"📊 [盘面日报] {report_date}", body, fname)
...
def report_already_sent(report_date):
    with db_lock:
        cur = db_conn.execute("SELECT email_ok FROM daily_report_status WHERE report_date=?", (str(report_date),))
        row = cur.fetchone()
        ...
        with db_lock:
            db_conn.execute("""
                ...
                ON CONFLICT(report_date) DO UPDATE SET
                  attempts=daily_report_status.attempts+1,
                  last_error=excluded.last_error
            """, (str(report_date), now_beijing().isoformat(), "", 0, 1, str(e)))
            db_conn.commit()
...
def main_loop():
    ...
    if not symbols or not high_volume_symbols:
        ...
        notify_all("❌ 系统启动失败：无法获取币种列表或成交额数据", "系统启动失败")
        return
    ...
    while True:
        try:
            ...
            logger.info(f"🟡 启动雷达完成 ({time.time()-t0:.1f}s)，观察池 {len(pending_early)}")

            # ② 验证与确认（内部收集信号到 pending_confirm_notifications）
            process_early_validation()

            # 处理待发送确认信号（爆发限制）
            if pending_confirm_notifications:
                pending_confirm_notifications.sort(key=lambda x: x.get("score", 0), reverse=True)
                strong_list = [s for s in pending_confirm_notifications if s.get("score", 0) >= STRONG_SCORE_MIN]
                normal_list = [s for s in pending_confirm_notifications if s.get("score", 0) < STRONG_SCORE_MIN]
                to_send = strong_list[:3] + normal_list[:max(0, 5-len(strong_list[:3]))]
                for sig in to_send:
                    # notify_signal 内部有阶段锁，这里无需重复检查，但为防止重复标记可手动
                    notify_signal(sig)
                pending_confirm_notifications.clear()
